...
discriminator.compile(loss=['binary_crossentropy','categorical_crossentropy'],loss_weights=[1., 0.15], optimizer=adam, metrics=['accuracy'])
...
gan.compile(loss=['binary_crossentropy','categorical_crossentropy'],loss_weights=[1., 0.15], optimizer=adam, metrics=['accuracy'])
gan.summary()
...
def train(d, epochs=1, plt_frq=1, BATCH_SIZE=128):
    ...
    for e in range(1, epochs + 1):
        ...
        for _ in range(batchCount):  # tqdm_notebook(range(batchCount), leave=False):
            ...
            ys = np.zeros((2 * BATCH_SIZE,classnum+1))
            ys[:BATCH_SIZE,0] = 1
            ys[BATCH_SIZE:2*BATCH_SIZE,(e%classnum)+1] = 1
            ...
            ys2 = np.zeros((BATCH_SIZE,classnum+1))
            
            ys2[:BATCH_SIZE,(e%classnum)+1] = 1
            discriminator.trainable = False
